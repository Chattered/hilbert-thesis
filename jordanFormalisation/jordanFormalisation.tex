...
\flushright{\cite{Veblenphd}, page 365.}
\end{quotation}
Looking at Veblen's proof attempt, one might suspect that manipulations of sequences of points in general configurations and reasoning about such sequences of points would dominate the argument, rather than synthetic constructions of diagrams. but this is not entirely the case. Our verification shows that there are a \emph{lot} of useful lemmas very similar to those we saw in Chapter~\ref{chapter:Group2Eval}, whose synthetic proofs obtain and then reason about properties of diagrams.
...
%As we described Hilbert's Theorem~6 (see \S\ref{sec:Theorem6}) as a meta-theorem, we would describe Veblen's argument here as a \emph{metaproof}. The use of such metatheoretical arguments in axiomatic geometry probably goes back to Euclid, who 
...
\flushright{\emph{Foundations of Geometry}~\cite{FoundationsOfGeometry}, pages 8-9.}  
\end{quote}
...
\flushright{\emph{Foundations of Geometry}~\cite{FoundationsOfGeometry}, page 9.}  
\end{quote}
