...
In the last chapter, we considered the theory of linear-ordering based on Hilbert's three-place $\code{between}$ relation. This culminated in a tactic for solving problems arranging points along a line by reducing them to a decision procedure for linear problems in natural numbers. The use of natural numbers was explicitly permitted in Veblen's ordered geometry~\cite{Veblenphd}, but not in Hilbert's. So in the last chapter, we showed how they can be recovered in higher-order logic from geometry without the axiom of infinity.
...
After stating that there are an infinite number of points on a line in THEOREM~7, Hilbert gives the following result and definitions:
...
While \emph{rays} make for a useful abstraction in later definitions and axioms (see earlier work on axiomatising Group~III~\cite{ScottMScThesis}), a four-place relation $\code{same\_side}$
relating the point $O$ and line $a$ with every other pair of points seems grossly unweildy. Besides, once we have a notion of rays, we can just say that two points are on the line $a$ on the same side of $O$ when they lie on a \emph{ray} lying on $a$ and emanating from~$O$.

We decided we would only formalise the notion of \emph{ray}, and we took a similar attitude for the two-dimensional analogue by introducing the notion of \emph{half-planes}. A \emph{half-plane}, then, is the totality of points on the same side of a line $a$ in the plane $\alpha$. To say that two points lie on the same side of the line $a$ is precisely to say that they lie on a single half-plane in $\alpha$ and bounded by $a$. This appears to take us away from Hilbert's prose, but in \S\ref{sec:HalfPlaneTranslations}, we explain how the prose rendering can easily be recovered from the formalisation.

\subsection{Rays}
It turned out, as we shall see in Chapters~\ref{chapter:JordanVerification1} and~\ref{chapter:JordanVerification2}, that the notion of a ray was not particularly useful in formal proofs. In this subsection, we will nevertheless justify keeping it in our formalisation. Rays are useful in Group~III, where Hilbert introduces an axiom governing congruence of angles:
...
\begin{gather}
  ...
  \begin{split}
    &\vdash\code{on\_ray\_of\_arrow}\ P\ \code{a}\\
...
The formal proof is shown in Figure~\ref{fig:SupplementIProof}. Our incidence discoverer from Chapter~\ref{chapter:Automation} helps keep the proof steps almost one to one with the prose. We start by concluding as Bernays does that the points $D$, $E$ and $F$ are distinct and then apply THEOREM~4 to show that one of the points lies between the other two.
...
\begin{figure}
  \begin{equation}\label{eq:halfPlaneOnPlane}
    \begin{split}
      &\code{half\_plane\_contains}\ hp\ P \wedge \code{on\_plane}\ P\ \alpha \\
  ...
  \begin{equation}\label{eq:onHalfPlaneNotBet}
    \begin{split}
      &(\forall R.\; \code{half\_plane\_contains}\ hp\ R \implies \code{on\_plane}\ R\ \alpha) \wedge \code{on\_half\_plane}\ hp\ P\\
  ...
  \begin{equation}\label{eq:betOnHalfPlane1}
    ...
    \begin{split}
      &\code{on\_line}\ P\ (\code{line\_of\_half\_plane}\ hp) \wedge \code{half\_plane\_contains}\ hp\ Q\\
  ...
  \begin{equation}\label{eq:betOnHalfPlane2}
    \begin{split}
      ...
      % ==> on_half_plane hp Q"
      &\code{half\_plane\_contains}\ hp\ P \wedge \code{on\_half\_plane}\ hp\ R\\
      &\implies \between{P}{Q}{R} \implies \code{half\_plane\_contains}\ hp\ Q
    \end{split}
  \end{equation}
\caption{Additional Theorems for Half-Planes}
...
\begin{multline}
\vdash\code{same\_half\_plane}\ x\ y \implies \code{half\_plane\_intermediate\_contains}\ x \\= \code{on\_half\_plane\_intermediate\_contains}\ y
\end{multline}
...
Many of the issues discussed so far in Group~II go some way to reinforcing the idea that Hilbert was not particularly concerned with ordered geometry. His axioms were highly redundant, and theorems such as THEOREM~8 are left without proof even as they require a fair amount of supplementary details. 
