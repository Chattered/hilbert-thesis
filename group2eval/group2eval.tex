...
When writing our verifications, we always tried our best to follow the specific steps in Hilbert's prose. In this chapter, we shall directly compare our verifications with the originals.
...
\flushright{Veblen~\cite{Veblenphd}}
\end{quotation}
...
The ``only one'' part is all that is retained in the tenth edition. The existence part is given in a proof which Hilbert credits to Wald (page 7 of the \emph{Foundations of Geometry}~\cite{FoundationsOfGeometry}).
\begin{quotation}
...
In this subsection, we give an idea of how our discoverers interact with HOL~Light by showing the sequents generated concurrently as we interactively develop a declarative verification of THEOREM~5. We consider two discoverers, \code{by\_incidence}, which generates the five kinds of basic incidence sequents described in the last chapter, and \code{by\_pasch}, which finds potential applications of Pasch's axiom. The \code{by\_pasch} discoverer feeds off the generations produced \code{by\_incidence}, so that their work is not duplicated. But there is no \emph{feedback}. That is, \code{by\_incidence} does not continue searching based on the results of \code{by\_pasch}. Instead, we shall take responsibility for how Pasch's axiom is used, since it is a point introduction axiom explicit in the prose that we want to keep explicit in the verification. Thus, we let search terminate at these applications.

The discovery begins once we have stated the theorem's assumptions and obtained our first non-collinear point obtained.
