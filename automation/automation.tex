...
Using the \code{accum} function as described in~\S\ref{sec:Accumulating}, we can gather up all theorems found so far across all of a discoverer's generations, continually storing the cumulative generation in a reference cell \code{the\_facts} which can be deferenced in the interactive proof. In this way, the user can make use of the results of concurrent discoverers as they come in.
