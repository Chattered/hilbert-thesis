...
\begin{quote}
THEOREM 5. Given any four points on a line, it is always possible to label them $A$, $B$, $C$, $D$ in such a way that the point labelled $B$ lies between $A$ and $C$ and also between $A$ and $D$, and furthermore, that the point labelled $C$ lies between $A$ ad $D$ and also between $B$ and $D$.
...
\subsubsection{Evaluating our Isabelle Verification}\label{sec:FindingAEH}
Our Isabelle verification of this proof runs to approximately 135 lines of complicated proof steps. As we should expect by now, most of these steps were used to derive the preconditions needed for Axiom~\ref{eq:g24}. In our HOL~Light verification, they are handled by our incidence discoverers.
...
The next sequence of generations is shown in Figure~\ref{fig:SecondGenerations} and is found within 1.21 seconds. Including the sequents found earlier, we have thirteen triangles identified in total, and so we are going to be interested in which applications of Axiom~\ref{eq:g24} are permissible. To find out, we look more specifically at the results of our \code{by\_pasch} discoverer. Already, it is telling us that there are six possibilities to choose from. The full set is shown in Figure~\ref{fig:PaschGenerations} and is found within 2.82 seconds.
...
The last possibility corresponds to one of Hilbert's applications of Axiom~II,~4. To apply it ourselves, all we need to do is assert the theorem as \code{clearly} derivable, being sure to supply all three hypotheses as justification:
...
We now have a disjunction in our hypotheses. When we run our discoverer with the \code{split} function, it will convert this disjunction into a tree, which will combine as described in \S\ref{sec:Trees}. Our generations become proper trees, and the search space is automatically partitioned into three. 
