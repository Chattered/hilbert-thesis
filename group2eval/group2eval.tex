...
In our other verification of THEOREM~5, which avoids the Inner and Outer Pasch axioms and follows Hilbert most closely, we apply Axiom~II, 4 via the \code{by\_pasch} discoverer. By doing so, we see there are exactly three applications implied by Hilbert's prose (we elide the subproofs used to reject offending disjuncts).
...
Finally, we mention Hilbert's final step ``one shows analogously that $B$ also lies on this segment.'' This does not require an analogous ``proof'' as the word ``show'' would imply. Instead, we can capture the analogy directly by setting the theorem verified in Figure~\ref{fig:FiveVerification1} as a lemma and then appling the symmetry of betweenness. In fact, \code{MESON} takes care of this automatically:
