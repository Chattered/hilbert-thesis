\chapter{Verification of the Polygonal JCT: Part I}\label{chapter:JordanVerification1}
...
With this second approach, we do not have to rotate our polygon, paving the way for a simple inductive verification of our sketch proof. The expression for the initial context appears in our specification of crossings, and the fact that this expression denotes a fixpoint is a lemma used in the verification of \label{eq:polypathCrossingsEven}.
...
%% If the break does not leave us with an empty list, then we consider the first vertex $P_3$ in the list which is distinct from $P_2$.
...
%   &\qquad\qquad \code{poly\_path\_connected}\ \alpha\ (\code{on\_polypath}\ Ps)\ P\ Q\\
%   &\qquad\qquad \vee \code{poly\_path\_connected}\ \alpha\ (\code{on\_polypath}\ Ps)\ P\ R\\
%   &\qquad\qquad \vee \code{poly\_path\_connected}\ \alpha\ (\code{on\_polypath}\ Ps)\ Q\ R
% \end{align*}
...
% \begin{itemize}
% \item
% Any two interior points of a triangle are polygonal path-connected.
% \item 
% A point in the interior of a triangle is \emph{not} polygonal path-connected to an exterior point.
% \item
% Any two exterior points of a triangle are polygonal path-connected.
% \end{itemize}
...
% \section{Interior points are not Polygonal Path-Connected to Exterior Points}
